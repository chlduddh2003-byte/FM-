...
# API 설정 (Streamlit Secrets에서 불러옴)
...
try:
    ...
    with col5:
        is_danger = current_vix >= 30
        card_class = "status-card status-success" if is_danger else "status-card"
        st.markdown(f"""
        <div class="{card_class}">
    ...
    st.error(f"대시보드 실행 중 오류 발생: {main_err}")
